...
public:
  ...
  float_t
  filterIn (float_t input);

  void flush ();
  void init (bool doFlush = true);

  void setSamplingTime (float_t ts_, bool doFlush = true)
  {
    ts = ts_;
    init (doFlush);
  }
  void setCutoffFreqHZ (float_t hz_, bool doFlush = true)
  {
    hz = hz_;
    init (doFlush);
  }
  void setOrder (ORDER od_, bool doFlush = true)
  {
    od = od_;
    init (doFlush);
  }
