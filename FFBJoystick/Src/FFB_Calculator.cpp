  ...
  switch (effect.effectType)
    ...
    case USB_EFFECT_SPRING://8
   	...
   	case USB_EFFECT_INERTIA://10
   			...
   			metric = InertiaFilterLp[axis]->process(_effect_params.inertiaAcceleration) * 6.0f;
   			angle_ratio = rotateConditionForce ? angle_ratio : 1.0;
   			if (_effect_params.inertiaAcceleration < 0 &&  _effect_params.frictionPositionChange < 0)
   			{
   				force =  ConditionForceCalculator(effect, abs(metric), 0.55f, condition) * angle_ratio * _gains.inertiaGain;
   			}
   			else if ( _effect_params.inertiaAcceleration < 0 && _effect_params.frictionPositionChange > 0)
   			{
					force =  -1 *  ConditionForceCalculator(effect, abs(metric), 0.55f, condition) * angle_ratio * _gains.inertiaGain;
				}
