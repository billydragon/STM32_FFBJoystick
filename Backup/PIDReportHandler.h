...
public:
  ...
  uint8_t
  GetNextFreeEffect (void);
  void
  StartEffect (uint8_t id);
  void
  StopEffect (uint8_t id);
  void
  StopAllEffects (void);
  void
  FreeEffect (uint8_t id);
  void
  FreeAllEffects (void);
  void
  sendStatusReport ();
  ...
  void
  CreateNewEffect (USB_FFBReport_CreateNewEffect_Feature_Data_t *inData);
  void
  UppackUsbData (uint8_t *data, uint16_t len);
  uint8_t*
  getPIDPool ();
  uint8_t*
  getPIDBlockLoad ();
